...
export function App(): ReactElement {
  ...
  const [healthList, setHealthList] = useState<Array<HealthDataShort>>(
    new Array<HealthDataShort>()
  );
  ...
  useEffect(() => {
    ...
    fetch("healthData/showAll")
      .then((response) => response.json())
      .then((data) => {
        var translateWeirdData: Array<HealthDataShort> =
          new Array<HealthDataShort>();
        ...
        // sort it by date
        translateWeirdData = translateWeirdData.sort(
    ...
    <div className="App">
      <header className="App-header">
        <div className="health-data-list">
          <HealthDataList
            healthDataList={healthList}
            setHealthDataList={setHealthList}
            setSelectedItem={fetchHealthData}
          />
        </div>
        <div className="health-data-side">
          <HealthSummary healthData={health} setHealthData={setHealth} />
          <MealsList
            healthDataId={health.idhe}
            meals={health.diet}
            setMeals={(newList) => setHealth({ ...health, diet: newList })}
          />
        </div>
        <div className="muscle-side">
          <div className="title">
            {viewSoreness ? "Muscle soreness" : "Workout routine"}
          </div>
          <Button
            variant="contained"
            onClick={() => setViewSoreness((prevState) => !prevState)}
          >
            Switch views
          </Button>
          {viewSoreness ? (
            <MuscleStatus
              parentId={health.idhe}
              isWorkout={false}
              enableEdit={true}
              intensityData={health.muscleGroups}
              setIntensityData={(data) =>
                setHealth({ ...health, muscleGroups: data })
              }
            />
          ) : (
            <WorkoutList
              healthDataId={health.idhe}
              workouts={health.workout}
              setWorkouts={(data) => setHealth({ ...health, workout: data })}
            />
          )}
        </div>
      </header>
